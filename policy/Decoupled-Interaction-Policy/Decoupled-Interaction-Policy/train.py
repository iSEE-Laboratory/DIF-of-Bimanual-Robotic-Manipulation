...
import sys
sys.path.insert(0, '../')
sys.path.append('Decoupled-Interaction-Policy/env_runner')
sys.path.append('Decoupled-Interaction-Policy/diffusion_policy/policy')
sys.path.append('Decoupled-Interaction-Policy')
sys.path.append('Decoupled-Interaction-Policy/diffusion_policy')

from hydra.core.hydra_config import HydraConfig
from diffusion_policy.policy.policy import DIP
...
class TrainDIPWorkspace:
    ...
    def __init__(self, cfg: OmegaConf, output_dir=None):
        ...
        # configure model
        self.model: DIP = hydra.utils.instantiate(cfg.policy)

        self.ema_model: DIP = None
    ...
    def get_policy_and_runner(self, cfg, checkpoint_num=3000):        
        ...
        if not cfg.policy.use_pc_color:
            ckpt_file = pathlib.Path(f'./policy/Decoupled-Interaction-Policy/Decoupled-Interaction-Policy/checkpoints/{self.cfg.task.name}/{checkpoint_num}.ckpt')
        else:
            ckpt_file = pathlib.Path(f'./policy/Decoupled-Interaction-Policy/Decoupled-Interaction-Policy/checkpoints/{self.cfg.task.name}_w_rgb/{checkpoint_num}.ckpt')
    ...
    def get_checkpoint_path(self, tag='latest'):
        if tag=='latest':
            ...
            all_checkpoints_guidance_cross_attention = os.listdir(checkpoint_dir)
            best_ckpt = None
            best_score = -1e10
            for ckpt in all_checkpoints_guidance_cross_attention:
                if 'latest' in ckpt:
                    continue
