import sys
sys.path.append('./policy/Decoupled-Interaction-Policy/Decoupled-Interaction-Policy')
sys.path.append('./')
...
@hydra.main(
    ...
    config_path=str(pathlib.Path(__file__).parent.joinpath(
        '../../policy/Decoupled-Interaction-Policy/Decoupled-Interaction-Policy/diffusion_policy', 'config'))
)
def main(cfg):
    global TASK
    TASK = cfg.task.name
    ...
    if checkpoint_num == -1:
        ...
        for i in range(10):
            ...
            dip = DIP(cfg, checkpoint_num)
            st_seed, suc_num = test_policy(task, args, dip, st_seed, test_num=test_num)
    ...
    if not cfg.policy.use_pc_color:
        save_dir  = f'decoupled_interaction/{TASK}'
    else:
        save_dir = f'decoupled_interaction/{TASK}_w_rgb'
...
def test_policy(Demo_class, args, dip, st_seed, test_num=20):
    global TASK
    epid = 0      
    seed_list=[]  
    suc_num = 0   
    ...
    for seed_id in range(test_num):
        ...
        Demo_class.apply_dp3(dip, video_log=False,
                             save_dir='decoupled_interaction')

        now_id += 1
        Demo_class.close()
        if Demo_class.render_freq:
            Demo_class.viewer.close()
        dip.env_runner.reset_obs()
